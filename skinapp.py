...
def load_model(model_path):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    # Assuming FixCapsNet is defined somewhere above in your code
    model = FixCapsNet(
        conv_inputs=3,  # Assuming RGB images
        conv_outputs=128,
        primary_units=8,
        primary_unit_size=16 * 6 * 6,  # Example size
        output_unit_size=16,
        num_classes=7,
        init_weights=True,
        mode='128'  # Assuming a mode from your configuration
    )
...
def mask_and_predict(image, model):
    ...
    if contours:
        ...
        masked_image = cv2.cvtColor(image_np, cv2.COLOR_BGR2RGB)

    pil_image = Image.fromarray(masked_image)
    oil_image = Image.fromarray(image_np)
    ...
    image_tensor = transform(oil_image).unsqueeze(0).to(device)
    print(image_tensor.shape)
    with torch.no_grad():
        output = model(image_tensor)
        v_mag = torch.sqrt(torch.sum(output**2, dim=2, keepdim=True))
        pred_class = v_mag.data.max(1, keepdim=True)[1].cpu().item()

    return masked_image, pred_class

model = load_model('last_path_87.pth')
...
if uploaded_file:
    ...
    if st.button('Analyze Lesion'):
        masked_image, lesion_type = mask_and_predict(image, model)
        st.image(masked_image, caption='Masked Lesion Image', use_column_width=True)
        st.write(f'Predicted Lesion Type: {lesion_type}')
